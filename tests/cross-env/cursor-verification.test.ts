...
suiteRunner('Cross-Env: Cursor IDE Verification (Issue #509)', () => {
  ...
  describe('C2: Tool Discovery & Listing', () => {
    ...
    test('Initial tools/list returns Tier 1 tools only (53 tools) + expand_tools', async () => {
      const { response } = await sendAndReceive(server, 'tools/list');
      tier1Tools = response.result.tools;
      // 53 Tier 1 tools (includes oc_reap_orphans lifecycle sweep, oc_assert,
      // oc_evidence_bundle, oc_skill_record, oc_skill_recall, oc_observe, crawl_start, crawl_status, crawl_cancel, and task ledger tools) + 1 expand_tools virtual tool = 54
      ...
      expect(nonExpandTools.length).toBe(53);
    });
