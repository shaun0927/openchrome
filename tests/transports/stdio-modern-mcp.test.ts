...
function createHarness(
  handler: (
    ...
    signal?: AbortSignal,
    context?: { mcpSessionId?: string; channel?: string },
...
describe('MCP 2026-07-28 stdio boundary', () => {
  ...
  test('keeps legacy initialize working as the local client without a session id', async () => {
    let observedSessionId: string | undefined = 'unset';
    let observedChannel: string | undefined = 'unset';
    let observedEra: string | undefined;
    ...
    const harness = createHarness(async (message, _signal, context) => {
      ...
      if (message.method === 'tools/list') {
        // The stdio peer is the local client: no MCP session id and no HTTP
        // channel, so the core keeps the historical `default` browser session.
        observedSessionId = context?.mcpSessionId;
        observedChannel = context?.channel;
        observedEra = currentRequestContext()?.protocolEra;
        return {
          jsonrpc: '2.0',
          id: message.id as number,
          result: { tools: [] },
    ...
    try {
      ...
      expect(observedChannel).toBeUndefined();
      expect(observedEra).toBe('legacy');
    } finally {
      await harness.transport.close();
    }
  });
