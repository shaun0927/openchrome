...
import { prependHeaderText, isStateHeaderEnabled } from './_shared/state-header';
  ...
  try {
    ...
    if (mode === 'dom') {
      try {
        ...
        if (compression === 'delta') {
          ...
          if (previous) {
            ...
            if (delta.isDelta) {
              ...
              const domDeltaPayload = statsLine + delta.content + nodeRefsBlock + domPaginationSection;
              return {
        ...
        const includePaginationDom = args.includePagination !== false;
        const domPaginationSection = includePaginationDom ? formatPaginationSection(await detectPagination(page, tabId)) : '';
        const domFullPayload = outputText + nodeRefsBlock + domPaginationSection;
        return {
    ...
    if (outputCharCount > MAX_OUTPUT) {
      ...
      try {
        ...
        const axFallbackPayload = domResult.content + fallbackNote + fallbackNodeRefsBlock + axPaginationSection;
        return {
          content: [
            {
              type: 'text',
