...
export class RecoveryTrajectoryLedger {
  ...
  /** Best-effort append. Returns the node when persisted, null when skipped/failed. */
  ...
  readRecent(limit = 50, sessionId?: string): RecoveryTrajectoryNode[] {
    try {
      ...
      const filtered = sessionId ? nodes.filter((n) => n.sessionId === sessionId) : nodes;
  ...
  private enforceNodeCountOnly(): void {
    const lines = safeReadLines(this.filePath);
    if (lines.length <= this.maxNodes) return;
    fs.writeFileSync(this.filePath, lines.slice(-this.maxNodes).join('\n') + '\n', 'utf8');
  }

  private rewriteTail(): void {
    const lines = safeReadLines(this.filePath).slice(-this.maxNodes);
    const kept: string[] = [];
    let bytes = 0;
