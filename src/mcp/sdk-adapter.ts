...
export interface SdkAdapterOptions {
  era: 'legacy' | 'modern';
  /**
   * Set for HTTP requests. Stdio requests carry no channel: the single stdio
   * client is OpenChrome's local client and keeps the historical `default`
   * browser session.
   */
  channel?: 'http';
  /** Advertised through initialize (legacy) and server/discover (modern). */
...
export function createSdkServerAdapter(
  ...
  const server = new Server(
    ...
    {
      capabilities: {
        ...(options.metadata?.experimental
          ? { experimental: options.metadata.experimental as ServerCapabilities['experimental'] }
          : {}),
