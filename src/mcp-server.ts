  ...
  private async handleInitialize(_params?: Record<string, unknown>): Promise<MCPResult> {
    return {
      ...
      instructions: [
        ...
        '',
        'PAGINATION RULE:',
        '- When extracting multi-page content (PDF viewers, slides, articles with pagination):',
        '  1. ALWAYS call read_page first — it detects pagination and reports total pages',
        '  2. If totalPages > 3, use batch_paginate (captures all pages in ONE call)',
        '  3. NEVER use manual ArrowRight+screenshot loops for >3 pages',
        '  4. batch_paginate supports: keyboard (ArrowRight), click (next button), url (parallel), scroll (infinite)',
        '',
        ...
        '  batch_paginate → Bulk-extract paginated content in one call',
      ].join('\n'),
    };
  }
