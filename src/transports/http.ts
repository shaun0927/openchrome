 ...
 * - 2026-07-28: stateless POST requests and subscriptions/listen, served by
...
import { MODERN_PROTOCOL_VERSION, rejectUnsupportedLegacyHttpVersion } from '../mcp/runtime-contract';
import { MCPResponse, MCPErrorCodes } from '../types/mcp';
import { ClientDisconnectError } from '../errors/abort';
import { MCPTransport, McpServerMetadata, TransportMessageContext } from './index';
...
export class HTTPTransport implements MCPTransport {
  ...
  onMessage(
    ...
    // Modern (2026-07-28) requests are served statelessly by the official SDK,
    // which dispatches each request into the same OpenChrome core handler.
    // Legacy traffic keeps the sessionful path below.
    this.modernHandler = createMcpHandler(
      ({ era }) => createSdkServerAdapter(handler, { era, channel: 'http', metadata: this.serverMetadata }),
      {
        legacy: 'reject',
        responseMode: 'auto',
  ...
  private hasModernProtocolHeader(req: http.IncomingMessage): boolean {
    ...
    return version?.trim() === MODERN_PROTOCOL_VERSION;
  }
  ...
  private handlePost(
    ...
    req.on('end', async () => {
      ...
      if (!legacyRequest) {
        // The SDK owns modern envelope, header, and version validation.
        req.setTimeout(0);
      ...
      const protocolError = rejectUnsupportedLegacyHttpVersion(req.headers['mcp-protocol-version']);
      if (protocolError) {
