 ...
 *
 * The stdio peer is OpenChrome's single local client. Requests from it carry
 * no MCP session id, so the core keeps the historical `default` browser
 * session and delivers background notifications to this transport.
 */
...
import type { MCPTransport, McpServerMetadata } from './index';
...
export class SdkStdioTransport implements MCPTransport {
  ...
  private readonly servers = new Map<string, { server: Server; era: 'legacy' | 'modern' }>();
  ...
  start(): void {
    ...
    this.handle = serveStdio(
      ({ era }) => {
        ...
        const server = createSdkServerAdapter(this.messageHandler!, { era, metadata: this.metadata });
        this.servers.set(key, { server, era });
  ...
  /**
   * Deliver a core notification to the legacy stdio client. Modern clients
   * receive change events only through subscriptions/listen, and request
   * scoped messages through the SDK request context.
   */
